...
try:
    ...
    # Try to import the ML verification module which uses BlueCarbon.pkl
    try:
        from ml_verification import ml_engine
        ML_VERIFICATION_AVAILABLE = True
        print("✅ ML verification module loaded (BlueCarbon model)")
    except Exception as ml_err:
        ML_VERIFICATION_AVAILABLE = False
        ml_engine = None
        print(f"⚠️ ML verification module not available: {ml_err}")
...
class ProductionAPIHandler(BaseHTTPRequestHandler):
    ...
    def _can_revert_project(self, project):
        """Check if a project can be reverted (max 3 times, status must be declined)"""
        return project.get('revert_count', 0) < 3 and project.get('status') == 'declined'

    def _revert_project(self, project, comment, admin_id):
        """Revert a project back to user for changes"""
        project['status'] = 'reverted_to_user'
        project['revert_count'] = project.get('revert_count', 0) + 1
        project['revert_comment'] = comment
        project['revert_by_admin'] = admin_id
        project['revert_timestamp'] = get_current_time().isoformat()
        return project
    ...
    def do_POST(self):
        ...
        try:
            path = self.path.split('?')[0]
            # Admin: revert project to user for changes
            if path.startswith("/api/admin/projects/") and path.endswith("/revert"):
                project_id = path.split('/')[-2]
                content_length = int(self.headers.get('Content-Length', 0))
                post_data = self.rfile.read(content_length)
                try:
                ...
                # Get project from NeonDB or in-memory
                project = None
                if NEONDB_AVAILABLE and db_service:
                ...
                if not project:
                    self.send_error_response(404, f"Project {project_id} not found")
                    return
                if not self._can_revert_project(project):
                    self.send_json_response({
                        "success": False,
                        "message": "Project cannot be reverted (already reverted 3 times or not declined)"
                    }, 400)
                    return
                updated_project = self._revert_project(project, comment, admin_id)
                if NEONDB_AVAILABLE and db_service:
                    db_service.update_project(project_id, updated_project)
                else:
                    projects_db[project_id] = updated_project
                self.send_json_response({
                    "success": True,
                    "message": "Project reverted to user for changes",
                    "project": updated_project
                })
                return

            # User: resubmit reverted project
            if path.startswith("/api/projects/") and path.endswith("/resubmit"):
                project_id = path.split('/')[-2]
                content_length = int(self.headers.get('Content-Length', 0))
                post_data = self.rfile.read(content_length)
                try:
                ...
                if not project:
                    self.send_error_response(404, f"Project {project_id} not found")
                    return
                if project.get('status') != 'reverted_to_user':
                    self.send_json_response({
                        "success": False,
                        "message": "Project is not in reverted state"
                    }, 400)
                    return
                # Update project details with new data
                for k, v in data.items():
                    if k not in ['id', 'status', 'revert_count', 'revert_comment', 'revert_by_admin', 'revert_timestamp']:
                        project[k] = v
            ...
            if path == "/api/projects/create":
                ...
                # Prefer ML model (BlueCarbon.pkl) if available
                if 'ML_VERIFICATION_AVAILABLE' in globals() and ML_VERIFICATION_AVAILABLE and ml_engine:
                    try:
                        print(f"🤖 Running ML verification for project: {data.get('project_name', 'Unknown')}")
                        ml_result = ml_engine.predict_quality_score(data)
                        if isinstance(ml_result, dict) and ml_result.get('status') in ('success', 'fallback'):
                            verification_score = int(round(float(ml_result.get('quality_score', 0))))
                            # store ML result in ai_verification_result so DB stores ML score
                            ai_verification_result = ml_result
                            print(f"✅ ML verification completed with score: {verification_score}")
                            
                            # ALSO run enhanced AI verification for detailed analysis
                            if AI_VERIFICATION_AVAILABLE and enhanced_ai_engine:
                                try:
                                    print(f"🤖 Running enhanced AI verification for detailed analysis: {data.get('project_name', 'Unknown')}")
                                    enhanced_ai_result = enhanced_ai_engine.verify_project_submission(data)
                                    if isinstance(enhanced_ai_result, dict):
                                        print(f"✅ Enhanced AI verification completed: Category: {enhanced_ai_result.get('category', 'unknown')}, Status: {enhanced_ai_result.get('status', 'unknown')}")
                                    else:
                                        print(f"⚠️ Enhanced AI verification returned invalid result: {type(enhanced_ai_result)}")
                                        enhanced_ai_result = None
                                except Exception as e:
                                    print(f"❌ Enhanced AI verification failed: {str(e)}")
                                    enhanced_ai_result = None
                        else:
                            print(f"⚠️ ML verification returned unexpected result, falling back")
                            ai_verification_result = None
                            enhanced_ai_result = None
                            verification_score = self._calculate_manual_score(data)
                    except Exception as e:
                        print(f"❌ ML verification failed: {str(e)}")
                        import traceback
                        traceback.print_exc()
                        enhanced_ai_result = None
                        # Fallback to enhanced_ai_engine or original ai_engine
                        if AI_VERIFICATION_AVAILABLE and enhanced_ai_engine:
                            try:
                                enhanced_ai_result = enhanced_ai_engine.verify_project_submission(data)
                            ...
                            verification_score = self._calculate_manual_score(data)
                elif AI_VERIFICATION_AVAILABLE and enhanced_ai_engine:
                    try:
                        print(f"🤖 Running enhanced AI verification for project: {data.get('project_name', 'Unknown')}")
                        enhanced_ai_result = enhanced_ai_engine.verify_project_submission(data)
                        if isinstance(enhanced_ai_result, dict):
                            verification_score = int(enhanced_ai_result.get('overall_score', 0))
                            print(f"✅ Enhanced AI verification completed: Score {verification_score}, Category: {enhanced_ai_result.get('category', 'unknown')}, Status: {enhanced_ai_result.get('status', 'unknown')}")
                        else:
                            print(f"⚠️ Enhanced AI verification returned invalid result: {type(enhanced_ai_result)}")
                            enhanced_ai_result = None
                            verification_score = self._calculate_manual_score(data)
                ...
                if NEONDB_AVAILABLE and db_service:
                    try:
                        ...
                        print(f"🔍 DEBUG: About to store verification data:")
                        print(f"  - ai_verification_result: {type(ai_verification_result)} - {ai_verification_result is not None}")
                        print(f"  - enhanced_ai_result: {type(enhanced_ai_result)} - {enhanced_ai_result is not None}")
                        if enhanced_ai_result:
                            print(f"  - enhanced_ai_result keys: {list(enhanced_ai_result.keys())}")
                            print(f"  - enhanced_ai_result score: {enhanced_ai_result.get('overall_score')}")
